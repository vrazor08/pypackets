...
def _send_af_packet(fd: socket.socket, buf: bytearray, pkt_size: int, sendmmsg, iov_max: int = 1024, flags: int = 0) -> int:
  ...
  if iov_max >= pkts_count: return sendmmsg(fd, buf, pkt_size, flags)
  for i in range(iterations):
    pkt_sent += sendmmsg(fd, buf[i*iov_max*pkt_size:(i+1)*iov_max*pkt_size], pkt_size, flags)
  end = (pkts_count) - (iterations*iov_max)
  if end > 0: pkt_sent += sendmmsg(fd, buf[iterations*iov_max*pkt_size:], pkt_size, flags)
  return pkt_sent
